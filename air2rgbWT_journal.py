...
citypath =  gb_path + r'/Codes/AirRGB//lib/global_megacity3.csv'
...
def allpixval(prodT, start_date, end_date, save_path):
    ...
    for date_m in rrule.rrule(rrule.MONTHLY, dtstart=start_date, until=end_date):
        for prodT.file in glob(os.path.join(prodT.path, '*.' + prodT.prod + '*' + '.Global')): # no .tif extension provided in WT files and .tif extension in OM files
            ...
            date_ym = int(prodT.file[-17:-11]) # MODIS, OMI both WT[-17:-11], PM[-21,-15]
            print date_ym
...
def rgb_param(aodval, angval, verts, annot = False, type='max', std = 'False'):
    ...
    city['aodcount'] = pd.read_csv(csv_save_path + 'df_GlobalcityWT_AODpmv2.csv', header=0).isnull().sum(axis=1)
    city['angcount'] = pd.read_csv(csv_save_path + 'df_GlobalcityWT_ANGpmv2.csv', header=0).isnull().sum(axis=1)
...
def AirRGB(year, vertsT, timetype = 'month' ):
    ...
    if timetype == 'year':
        ...
        srs.ndarr_to_raster(rgbarr, aod.georef, 'AnnualRGB'+str(year) + '.tif')
        print 'saved ', year
    ...
    if timetype == 'month':
        ...
        for date_m in rrule.rrule(rrule.MONTHLY, dtstart=start_date, until=end_date):
            ...
            try:
                ...
                srs.ndarr_to_raster(rgbarr, aod.georef, 'MonthRGB' + str(file_date) + '.tif')
                # plt.figure()
                # plt.imshow(rgbarr)
    ...
    if timetype == 'daily':
            ...
            try:
                ...
                srs.ndarr_to_raster(rgbarr, aod.georef, 'DailyRGB' + str(year) + '.tif')
                # plt.figure()
                # plt.imshow(rgbarr)
...
def RGBcity(aodval, angval, citychk, vertsT):
...
def AirRGBdf_trend(aodval, angval, vertsT):
    ...
    for citychk in aodval.city:
        ...
        continent = aodval[aodval.city==citychk]['continent'].tolist()[0]
        ...
        ls.append([continent, citychk, fnregress(x0, y1)[0:2],fnregress(x0, y2)[0:2], fnregress(x0, y3)[0:2]])

    #save scale and offset as dataframe
    df_mc = pd.DataFrame(ls, columns = ['continent','city', 'Rmc', 'Gmc', 'Bmc'])
    ...
    #  save m and c as csv
    df_mc.to_csv(csv_save_path+'df_GlobalcityWT_RGBmc.csv', index=False, header=True)

    return df_mc
# fucntion end

def plotRGBmc(df_mc, labelc):
    color_dict = {'Europe': 'red', 'Africa': 'red', 'SouthAmerica': 'red', 'Asia': 'red', 'Oceania': 'red', 'NorthAmerica': 'red'}
    df_mc['color'] = df_mc['continent'].apply(lambda x: color_dict[x])
    ...
    ax.plot(x, y, 'o', color = labelc, label= labelc, alpha = 0.4, markersize=4)
...
#save_path_o = csv_save_path + 'df_GlobalcityWT_'+aod.prod+'wtv2.csv'
#save_path_n = csv_save_path + 'df_GlobalcityWT_'+ang.prod+'wtv2.csv'
...
aodval = pd.read_csv(csv_save_path + 'df_GlobalcityWT_AODwtv2.csv', header=0)
angval = pd.read_csv(csv_save_path + 'df_GlobalcityWT_ANGwtv2.csv', header=0)
...
triangle_gen(vertsT, verts_wtmn, 1100)
...
df_RGB.to_csv(csv_save_path + '/df_GlobalcityWT_trend.csv', index=False, header=True)

#       finding trend from the csv saved in previous step
df_RGB = pd.read_csv(csv_save_path + '/df_GlobalcityWT_trend.csv', header=0)
plotRGBtrend('Tokyo', df_RGB )


# ----------------------------------------------------------------
#   5. funcion run to find RGB trend for a city from AOD,ANG datatframes and plot. Useful for experimenting vertsS
citychk = 'NewDelhi'
RGBcity(aodval, angval, citychk, verts_wtmn)
