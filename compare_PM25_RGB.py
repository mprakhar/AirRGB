...
def allpixval(prodT, start_date, end_date, save_path, citychk):
    ...
    for date_m in rrule.rrule(rrule.DAILY, dtstart=start_date, until=end_date):
        for prodT.file in glob(os.path.join(prodT.path, '*.' + prodT.prod + '*' + '.Global')): # no .tif extension provided in WT files and .tif extension in OM files
            ...
            if ((str(date_ym) == (str(date_m.year) + str('%03d' %date_m.timetuple().tm_yday))) & os.path.isfile(prodT.file)):
...
def df_AQgen(df_AQ, vertsT):
    ...
    #df_AQ[['R', 'G', 'B']] = df_AQ.apply(lambda row: distRGB(xx=row['AOD'], yy=row['ANG'], vertsT=vertsT), axis=1)

    return df_AQ
#function end

# compute RGB values of city if AOD ANG already exist in a daatframe
def RGBcity(df_AQ_citychkpath, df_AQRGB_citychksavepath):
    #       vertices as found by wt data simple mean

    #       Deciding vertices of the RGB gradient triangle . order: B,R,G
    vertsT = [[50, 50], [800, 1000], [20, 1000]]

    # read df that has AOD and ANG fro city chek concerned
    df_AQin = pd.read_csv(df_AQ_citychkpath, header=0)

    #get rid of nan if any
    df_AQin.fillna(0, inplace=True)
    ...
    df_AQ.to_csv(df_AQRGB_citychksavepath, index=False, header=True)

    return df_AQ



# ---------   R U N   T H E  F U N C T I O N S
# run the functions
...
citychk ='Kanpur'
save_path_aod =  csv_save_path+'df_'+'AOD'+citychk+'0.csv'
save_path_ang =  csv_save_path+'df_'+'ANG'+citychk+'0.csv'
aod.path = '/home/wataru/research/MODIS/MOD04L2/L3/'
ang.path = '/home/wataru/research/MODIS/MOD04L2/L3/'

#First prepare the individual AOD and ANG
allpixval(aod, start_date, end_date, save_path_aod, citychk)
allpixval(ang, start_date, end_date, save_path_ang, citychk)
...
#manually merge the save_path_aod and save_path_ang inthe df_AQ_citychkpath
df_AQ_citychkpath = csv_save_path+'df_AQ'+citychk+'.csv'
#output path of AQRGB_daily
df_AQRGB_citychksavepath = csv_save_path+'df_AQRGB_daily'+citychk+'.csv'
# RUNt eh function
df_AQ = RGBcity(citychk, df_AQ_citychkpath, df_AQRGB_citychksavepath)
...
# ---------    O P E R A T I N G   O N   PM2,5 data
# also read PM
ndf = get_PM(citychk)
